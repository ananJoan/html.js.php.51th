<?php
    ...
    if(!empty($_POST)){
        ...
        if($_SESSION["nwpg"]==$_SESSION["alls"]){
            ...
            header("location:pen3.php?nows=$adds1");
        }
    }
?>
<!DOCTYPE html>
...
<body>
    <form action="pen3.php?nows=1" method="post">
    <table border="1" >
    <?php
        $allpgcount=ceil($count/1);
        ...
        $row=mysqli_query($db,"SELECT * FROM `question` WHERE `invide` LIKE '$inv' ORDER BY `num` ASC limit $sst,1");
